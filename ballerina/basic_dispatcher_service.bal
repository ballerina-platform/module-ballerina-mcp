...
type BasicDispatcherService distinct service object {
    *http:Service;

    isolated function addServiceRef(BasicMcpService basicMcpService);
    isolated function removeServiceRef();
    isolated function setServerConfigs(ServerConfiguration serverConfigs);
};

BasicDispatcherService basicDispatcherService = isolated service object {
    private ServerConfiguration? serverConfigs = ();
    private BasicMcpService? basicMcpService = ();
    private boolean isInitialized = false;
    private string? sessionId = ();

    isolated function addServiceRef(BasicMcpService basicMcpService) {
        lock {
            self.basicMcpService = basicMcpService;
        }
    }

    isolated function removeServiceRef() {
        lock {
            self.basicMcpService = ();
        }
    }
        ...
        lock {
            ...
            if request is JsonRpcRequest {
                if request.method == "initialize" {
                    ...
                    ListToolsResult listToolsResult = check self.executeListTools();
                    ...
                    CallToolResult callToolResult = check self.executeCallTool(params);
                    return <http:Ok>{
                        headers: {
                            [SESSION_ID_HEADER]: self.sessionId ?: ""
                        },
    ...
    private isolated function executeListTools() returns ListToolsResult|error {
        lock {
            BasicMcpService? chatService = self.basicMcpService;
            if chatService is BasicMcpService {
                return check listToolsForRemoteFunctions(chatService);
            }
            return error("MCP Service is not attached");
        }
    }

    private isolated function executeCallTool(CallToolParams params) returns CallToolResult|error {
        lock {
            BasicMcpService? chatService = self.basicMcpService;
            if chatService is BasicMcpService {
                return check callToolForRemoteFunctions(chatService, params.cloneReadOnly());
            }
            return error("MCP Service is not attached");
        }
    }
